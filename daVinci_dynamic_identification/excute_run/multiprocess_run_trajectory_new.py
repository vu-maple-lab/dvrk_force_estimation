...
# ros init node
ral = crtk.ral('dvrk_data_collection')
...
#   
    command = "rosbag record -O {0}_{1}.bag /{1}/measured_js /{1}/jaw/measured_js /{1}/spatial/jacobian".format(os.path.join(sub_folder_name, args.file_json.split('.')[0]),'PSM'+str(PSM_id))
...
# create arm
arm = arm_custom(ral, device_namespace=args.arm, expected_interval=0.001)
