...
class indirectTestDataset(indirectDataset):
    def __init__(self, path, window, skip, indices=[0, 1, 2, 3, 4, 5], is_rnn=False, filter_signal=False):
        super(indirectTestDataset, self).__init__(path, window, skip, indices=[0, 1, 2, 3, 4, 5], is_rnn=is_rnn,
...
class indirectTrocarDataset(indirectDataset):
    def __init__(self, path, window, skip, indices=[0, 1, 2, 3, 4, 5], num=1e9, seal='seal', filter_signal=False,
                 net='ff'):
        super(indirectTrocarDataset, self).__init__(path, window, skip, indices=[0, 1, 2, 3, 4, 5], is_rnn=False,
                                                    filter_signal=filter_signal)
        self.fs_pred = np.loadtxt(path+'/'+'TO_free_comp_model_pred.csv').astype('float32')
...
class indirectTrocarTestDataset(indirectTrocarDataset):
    def __init__(self, path, window, skip, indices=[0, 1, 2, 3, 4, 5], seal='base', filter_signal=False, net='ff'):
        super(indirectTrocarTestDataset, self).__init__(path, window, skip, indices=[0, 1, 2, 3, 4, 5], seal=seal,
                                                        filter_signal=filter_signal, net=net)
